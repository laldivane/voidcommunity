  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="text-xs uppercase tracking-widest text-crimson font-bold hover:text-ember transition-colors flex items-center gap-1 group">
          See All
...
function TrackCard({ track, listeners, gradient }: TrackCardProps) {
  const { playTrack, currentTrack, isPlaying } = useAudioStore();
  const isCurrentPlaying = currentTrack?.id === track.id && isPlaying;
...
export function MainFeed() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 overflow-y-auto scroll-smooth h-full flex items-center justify-center">
  ...
  return (
    <div className="flex-1 overflow-y-auto scroll-smooth h-full custom-scrollbar">
      ...
      <div className="max-w-5xl mx-auto px-8 py-10 space-y-12 pb-32">
        {/* Hero Banner (Always based on first fetch or search result) */}
        {filteredTracks.length > 0 && <HeroBanner track={filteredTracks[0]} />}

        {/* Search Results / Full Feed */}
        {filteredTracks.length > 0 && (
          <Section title={searchQuery ? "Matches from the Abyss" : "Echos of the Void"} delay={0.1}>
        ...
        {filteredCommunities.length > 0 && (
          <Section title="Sanctuary Spaces" delay={0.2}>
